...
Features:
  ...
  • TFT-inspired health scoring + RUL estimation
  • Alert engine with threshold rules
  • Full PostgreSQL / SQLite persistence
  • WebSocket live stream (1.5 s interval)
...
def get_laptop_metrics() -> dict:
    ...
    try:
        ...
        pass
    if temp_c is None:
        # Estimated from CPU load when real sensor unavailable
...
def _parse_meminfo(text: str, key: str) -> Optional[float]:
    for line in text.splitlines():
        if line.startswith(key):
            try:
                return float(line.split()[1])   # value in kB
            except Exception:
                pass
    return None
...
def get_phone_metrics() -> dict:
    ...
    bat_plugged  = bool(bat_raw) and ("AC powered: true" in bat_raw or "USB powered: true" in bat_raw)

    mem_raw   = _adb("cat /proc/meminfo")
    mem_total = _parse_meminfo(mem_raw, "MemTotal:")
    mem_avail = _parse_meminfo(mem_raw, "MemAvailable:")
    ram_pct   = round((1 - (mem_avail or 1) / (mem_total or 1)) * 100, 1) if mem_total else 60.0
    ...
    return {
        ...
        # Fallback simulated values when ADB is not connected
...
# ════════════════════════════════════════════════════════════════════════════
#  HEALTH SCORING  (TFT-inspired weighted model)
# ════════════════════════════════════════════════════════════════════════════
def compute_health(metrics: dict) -> dict:
    ...
    if dev == "laptop":
        ...
        temp_s = 1 - min(max(tc - 30, 0) / 70, 1)          # 30 °C → 1.0, 100 °C → 0.0
        ...
        raw = (W["cpu"]*cpu_s + W["ram"]*ram_s + W["temp"]*temp_s +
               W["bat"]*bat_s + W["disk"]*dsk_s + W["swap"]*swp_s)
        ...
        failures = dict(
            battery_cell_degradation = round(min(0.95, (100 - metrics.get("battery_percent", 100)) / 100 * 0.6
                                                 + (1 - components["thermal_health"] / 100) * 0.2), 3),
            thermal_throttling       = round(min(0.95, (1 - components["thermal_health"] / 100) * 0.8), 3),
            storage_sector_failure   = round(min(0.95, metrics.get("disk_used_pct", 50) / 100 * 0.3), 3),
            memory_leak              = round(min(0.95, metrics.get("ram_percent",   50) / 100 * 0.4), 3),
        )
        ...
        temp_s = 1 - min(max(tc - 20, 0) / 45, 1)          # 20 °C → 1.0, 65 °C → 0.0
        ...
        raw = (W["cpu"]*cpu_s + W["ram"]*ram_s + W["temp"]*temp_s +
               W["bat"]*bat_s + W["sig"]*sig_s + W["sto"]*sto_s)
        ...
        failures = dict(
            battery_swelling     = round(min(0.95, max(0, (tc - 35) / 30 * 0.6
                                                       + (100 - metrics.get("battery_percent", 75)) / 100 * 0.3)), 3),
            overheating_shutdown = round(min(0.95, (1 - components["thermal_health"] / 100) * 0.7), 3),
            storage_corruption   = round(min(0.95, metrics.get("storage_used_pct", 50) / 100 * 0.35), 3),
            ram_overflow         = round(min(0.95, metrics.get("ram_percent",       60) / 100 * 0.45), 3),
        )

    score = round(raw * 100)

    # RUL — exponential degradation model over recent history
    if len(metric_history) >= 5:
        recent_scores = [h.get("health_score", score) for h in list(metric_history)[-5:]]
        deg = max(0.001, (recent_scores[0] - recent_scores[-1]) / len(recent_scores))
    else:
        deg = 0.5

    rul   = min(730, max(0, round(score / max(deg, 0.1))))
    risk  = ("LOW"      if score >= 75 else
             "MEDIUM"   if score >= 55 else
             "HIGH"     if score >= 35 else "CRITICAL")
...
_THRESHOLDS = {
    "laptop": [
        ...
        ("battery_percent",  5, "critical", "Battery critically low — connect charger"),
...
def switch_device(device_type: str):
    ...
    if device_type not in ("laptop", "phone"):
        return JSONResponse({"error": "Use 'laptop' or 'phone'"}, status_code=400)
    current_device = device_type
    _last_alert_state.clear()
    return {"switched_to": current_device, "message": f"Now monitoring {device_type}"}
...
def health_score():
    m = get_laptop_metrics() if current_device == "laptop" else get_phone_metrics()
...
def db_metrics(
    ...
    if hours:
        q = q.filter(MetricRecord.timestamp >= datetime.utcnow() - timedelta(hours=hours))
...
def db_health(
    ...
    if hours:
        q = q.filter(HealthRecord.timestamp >= datetime.utcnow() - timedelta(hours=hours))
    rows = q.limit(limit).all()
    return {"count": len(rows), "records": [row_to_dict(r) for r in rows]}
...
def db_alerts(
    ...
    if device:              q = q.filter(AlertRecord.device_type == device)
    if severity:            q = q.filter(AlertRecord.severity    == severity)
    if unacknowledged_only: q = q.filter(AlertRecord.acknowledged == False)
    rows = q.limit(limit).all()
...
def db_stats(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "critical_alerts"      : db.query(func.count(AlertRecord.id))
                                   .filter(AlertRecord.severity == "critical").scalar(),
