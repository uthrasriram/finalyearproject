...
DATABASE_URL = os.getenv(
    "DATABASE_URL",
    "sqlite:///./device_health.db"          # default: local SQLite
    # "postgresql://health_user:health@1234@localhost:5432/device_health_db"
)
...
def save_alerts(db: Session, device_type: str, alerts: list):
    for a in alerts:
        if db.query(AlertRecord).filter(AlertRecord.alert_id == a["id"]).first():
            continue
        db.add(AlertRecord(
            alert_id    = a["id"],
            device_type = device_type,
